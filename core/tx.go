...
func TxpoolToTrTest(TxStr string) (TxBST,MineResult) {
	var txData TxData
	var tbst TxBST
	var mresult MineResult
	...
	txData.Timestamp=int(time.Now().Unix())
	txData.From="CdPvSHft7ZF42ksyJc6JDuBZkDD8gRuWAj"
	txData.To="CMwhM5iH9kU2TZLP1EmFcJD23CBfACMENK"
	txData.Amount=0.1
	txData.Fee=0.0001
	txData.Sign=setHash(setHash(txData.From))
	txData.Datatype="QUB"
	txData.Message="TxNode"
	...
	for i:=1;i<101;i++ {
		...
		mresult.Txamount=math.Round(mresult.Txamount*100000000)/100000000
		mresult.Sumfee=math.Round(mresult.Sumfee*100000000)/100000000
	}
...
func TxpoolToTr(TxStr string,blockno int) (TxBST,MineResult) {
	...
	for _,v:=range line {
		...
		if txc>0 && txData.Datatype!="Contract" && txData.Datatype!="Data" {
			//txData.Amount=txData.Amount/float64(txc)
